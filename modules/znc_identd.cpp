...
struct IdentPortKey
{
	const CIRCNetwork *netkey;
	const CUser       *userkey;

	bool operator<(const IdentPortKey &other) const { return (netkey < other.netkey || (netkey == other.netkey && userkey < other.userkey)); }
};
...
public:
	...
	map<IdentPortKey, IdentPortInfo> m_PortsByUsername;
	...
	{
		CTable tIdentTable;
		tIdentTable.AddColumn("ZNC User");
		tIdentTable.AddColumn("User Type");
		tIdentTable.AddColumn("Network");
		tIdentTable.AddColumn("Local Port");
		tIdentTable.AddColumn("Remote Port");
		tIdentTable.AddColumn("Ident Response");
		for (map<IdentPortKey, IdentPortInfo>::iterator i = m_PortsByUsername.begin(); i != m_PortsByUsername.end(); ++i)
		{
			const CUser *u = i->first.userkey;
			tIdentTable.AddRow();
			tIdentTable.SetCell("ZNC User", u->GetUserName());
			tIdentTable.SetCell("User Type", (u->IsAdmin() ? "admin" : ""));
			tIdentTable.SetCell("Network", i->first.netkey->GetName());
	...
	// this hook is called by CIRCSock's Connected() hook, which makes it exactly what we want (though we don't use it exactly the way it's named)
	{
		...
		IdentPortInfo &ipi = m_PortsByUsername[GetIdentKey()];
	...
	virtual void OnIRCDisconnected() {
		...
		map<IdentPortKey, IdentPortInfo>::iterator i = m_PortsByUsername.find(GetIdentKey());
		...
		// push the ident cancellation to the daemon, erase from the table
                ...
                m_PortsByUsername.erase(GetIdentKey());
	}

	IdentPortKey GetIdentKey()
	{
		IdentPortKey i;
		i.userkey = m_pUser;
		i.netkey = m_pNetwork;
